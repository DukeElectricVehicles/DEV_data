...
%filenames = sprintfc('../olga1.TXT',0);
filenames = sprintfc('../patrick.TXT',0);
...
figure;
vr = voltage - emf;
scatter(current(vr < 2), vr(vr < 2), 2, velo(vr < 2)); hold on; grid on; colorbar;
xlabel('Current in A');
ylabel('BMS voltage - emf');

coeffs = polyfit(current(vr < 2), vr(vr < 2), 1)
isweep = linspace(-3, 5, 1000);
plot(isweep, polyval(coeffs, isweep));
