...
filenameFormat = '12V_(?<mode>.?)(?<currentAcc>\d)A_(?<currentDec>-\d)A_(?<dummy>.*)_\d\.txt';
...
if (isRegen)
    targetCurrent = str2num(allParameters(1).currentDec)
else
    targetCurrent = str2num(allParameters(1).currentAcc)
end
