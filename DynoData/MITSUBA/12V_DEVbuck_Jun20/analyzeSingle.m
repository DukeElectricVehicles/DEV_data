function [Rs, Kv] = analyzeSingle(filePath, linecolor, linestyle, toPlot, duty)
...
%     ePower = power;
    ...
    ePower = smooth(ePower, 600, 'sgolay');

    accel = gradient(omega_fly)./gradient(time);
    accel(end-5:end) = accel(end-6);
    smooth(accel, HALLTEETH);
    assert(~any(isnan(accel)),'nans in accel');
    accel = smooth(time, accel, 1001, 'sgolay');
...
%     assert (abs(CoRV(3)) < 0, 'y intercept problem');
    ...
    if (toPlot)
        figure(1);
    %     yyaxis left
        plot(rpm_motor, eff, 'Color',linecolor, 'DisplayName', filename, 'LineStyle',linestyle); hold on;
    %     yyaxis right
