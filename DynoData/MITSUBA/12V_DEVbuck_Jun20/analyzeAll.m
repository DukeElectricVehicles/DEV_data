...
clf(1);clf(2);clf(3);clf(4);clf(5);clf(6);
...
%     stuff = struct('name',filename(1:end-q    6));
    if (length(stuff)~=1)
        continue;
    end
%     if (num2str(stuff.current) ~= 1)
%         continue
%     end
...
%     stuff = struct('name',filename(1:end-6));
    ...
    linecolor = allPlotColors(fix(str2num(stuff.current)),:);
%     linecolor = allPlotColors(find(ismemberstruct(allParameters,stuff)),:);
    filePath = strcat(filesStruct(i).folder, '/', filesStruct(i).name);
    
    [Rs, Kv] = analyzeSingle(filePath, linecolor, linestyle,true, 1);%str2num(stuff.duty));
    allRs = [allRs; Rs];
    allKv = [allKv; Kv];
end
...
%     allP = {fields{1:end-1};vals{1:end-1}};
