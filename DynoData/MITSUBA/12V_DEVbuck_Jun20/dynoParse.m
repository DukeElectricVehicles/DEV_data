...
ePower = smooth(ePower, 100);

accel = gradient(omega)./gradient(time);
accel(end-5:end) = accel(end-6);
assert(~any(isnan(accel)),'nan');
accel = smooth(accel, HALLTEETH);

% accel = smooth(rpm,accel, 20, 'sgolay');
accelOld = accel;
accel = smooth(accel, 500);
accelInterp = polyfit(omega,accel,10);
accel = polyval(accelInterp,omega);
...
figure(2);clf;
plot(rpm_motor, accelOld  - polyval(PARASITIC_LOSSES_ACC_OF_FLYWHEEL_RPS, omega)); hold on;
...
ylim([0.6, 1]); xlim([0,max(rpm_motor)]);
ylabel('efficiency');
xlabel('speed (mph)');
grid on;
...
figure(4);clf;
% plot(ePower, eff, '.','MarkerSize' ,1); hold on;
scatter(ePower,eff,1,1:length(ePower));colorbar
ylim([0.6, 1]);
if (isRegen)
    xlim([-100,0]);
else 
